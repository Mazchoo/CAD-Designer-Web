...
function filenamePlugin() {
  return {
    ...
    transform(code, id) {
      return {
        code: code.replaceAll('__DIRNAME__', () => `${JSON.stringify(makeRelativeToCWD(path.dirname(id)))}`),
        map: { mappings: '' },
      };
    },
  };
...
sampleFiles.filter((n) => n.endsWith('/index.html')).forEach((n) => writeRedirect(n));

const samplePlugins = [wgslPlugin(), nodeResolve(), commonjs(), typescript({ tsconfig: './sample/tsconfig.json' })];
...
export default [
  {
    ...
    plugins: [nodeResolve(), commonjs(), filenamePlugin(), typescript({ tsconfig: './src/tsconfig.json' })],
    watch: {
      clearScreen: false,
    },
  },
